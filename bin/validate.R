...
hold <- make.predictions(siamcat=sc.tr, siamcat.holdout=hold, normalize.holdout=TRUE)
hold <- evaluate.predictions(hold)

MET <- calculate_eval_matrix(hold, threshold = opt$threshold)
MET[["model_id"]] <- opt$prefix

fwrite(MET, file.path(opt$outdir, sprintf("validation_metrics_%s.csv", opt$prefix)))
suppressWarnings(model.evaluation.plot(hold,
  fn.plot = file.path(opt$outdir, sprintf("validation_evaluation_%s.pdf", opt$prefix))))
